#!/bin/bash
#SBATCH --job-name=bias_search_mpi
#SBATCH --nodes=4                    # 4 nodes as requested
#SBATCH --ntasks-per-node=8          # 8 MPI ranks per node (32 total ranks)
#SBATCH --cpus-per-task=24           # 24 CPU cores per rank (8*24=192 cores per node)
#SBATCH --time=0-04:00:00            # 4 hours max runtime
#SBATCH --output=bias_search_mpi_%j.out
#SBATCH --error=bias_search_mpi_%j.err
...
# Create a hostfile for MPI (useful for debugging)
...
OUTPUT_DIR=${OUTPUT_DIR:-"./mpi_results_$SLURM_JOB_ID"}

echo "Bias Search Parameters:"
...
echo "Starting MPI Bias Search at $(date)"
echo "========================================="
...
srun --cpu-bind=cores --kill-on-bad-exit=1 \
     ...
     --log_file "$OUTPUT_DIR/bias_search_mpi_$SLURM_JOB_ID.log"

echo "========================================="
echo "MPI Bias Search completed at $(date)"
